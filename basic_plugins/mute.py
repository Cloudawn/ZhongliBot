...
from src.utils.function import get_message_img, get_message_text, is_number
from src.utils.http_utils import AsyncHttpx
from src.utils.image_utils import get_img_hash
from src.utils.log import logger

__zx_plugin_name__ = "刷屏禁言 [Admin]"
__plugin_usage__ = f"""
usage：
    刷屏禁言相关操作，需要bot有群管理员权限
    指令：
        设置刷屏检测时间 [秒]
        设置刷屏检测次数 [次数]
        设置刷屏禁言时长 [分钟]
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if not mute_dict.get(event.user_id):
        mute_dict[event.user_id] = {
            "time": time.time(), "count": 1, "msg": msg}
    else:
