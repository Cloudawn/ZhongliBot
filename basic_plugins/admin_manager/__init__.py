...
# ticket管理器
ticket = on_regex(pattern=r"^ticket$", rule=check_event(),
                  permission=SUPERUSER, priority=2, block=True)
friend_list = on_regex(pattern=r"^好友列表$", rule=check_event(
), permission=SUPERUSER, priority=3, block=True)  # 好友列表
# 删除好友
friend_delete = on_regex(
    pattern=r"^删除好友 [\d]+$", rule=check_event(), permission=SUPERUSER, priority=3, block=True)
group_list = on_regex(pattern=r"^群列表$", rule=check_event(
), permission=SUPERUSER, priority=3, block=True)  # 群列表
group_delete = on_regex(pattern=r"^退群 [\d]+$", rule=check_event(),
                        permission=SUPERUSER, priority=3, block=True)  # 退群
borodcast = on_regex(pattern=r"^广播 [\d]+ ", rule=check_event(),
                     permission=SUPERUSER, priority=3, block=True)  # 广播
borodcast_all = on_regex(pattern=r"^全体广播 ", rule=check_event(
), permission=SUPERUSER, priority=3, block=True)  # 全体广播
# 打开关闭机器人
handle_robot = on_regex(
    pattern=r"^(打开|关闭) [0-9]+$", rule=check_event(), permission=SUPERUSER | GROUP_ADMIN | GROUP_OWNER, priority=3, block=True)
help = on_regex(pattern=r"^帮助$", rule=check_event(),
                permission=SUPERUSER, priority=3, block=True)  # 帮助
# 添加ticket
ticket_add = on_regex(
    pattern=r"^添加 [^\s]+$", rule=check_event(), permission=SUPERUSER, priority=3, block=True)
# 删除ticket
ticket_del = on_regex(
    pattern=r"^删除 [\d]+$", rule=check_event(), permission=SUPERUSER, priority=3, block=True)
# 清理ticket
ticket_clean = on_regex(pattern=r"^清理$", rule=check_event(),
