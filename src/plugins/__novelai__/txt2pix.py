...
txt2pix = on_command(".aidraw", aliases={"文本生图", "咏唱", "画"})
...
async def txt2pix_handle(event: GroupMessageEvent, args: Message = CommandArg()):
    ...
    if config.novelai_limit:
        ...
        await txt2pix.send(
            f"排队中，你的前面还有{list_len}人，请稍安勿躁" if list_len > 0 else "请稍等，图片生成中"
        )
