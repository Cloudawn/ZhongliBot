...
release_url = "https://github.com/Cloudawn/ZhongliBot/releases/latest"
...
async def check_update(bot: Bot) -> 'int, str':
    ...
    if data:
        ...
        if _version != latest_version:
            ...
            logger.info(f"检测钟离已更新，当前版本：{_version}，最新版本：{latest_version}")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]),
                message=f"检测钟离已更新，当前版本：{_version}，最新版本：{latest_version}\n" f"开始更新.....",
            )
            logger.info(f"开始下载钟离最新版文件....")
            ...
            if await AsyncHttpx.download_file(tar_gz_url, zhenxun_latest_tar_gz):
                ...
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
                    message=Message(f"钟离更新完成，版本：{_version} -> {latest_version}\n"
                ...
                logger.warning(f"下载钟离最新版本失败...版本号：{latest_version}")
                await bot.send_private_msg(
                    user_id=int(list(bot.config.superusers)[0]),
                    message=f"下载钟离最新版本失败...版本号：{latest_version}.",
                )
        else:
            logger.info(f"自动获取钟离版本成功：{latest_version}，当前版本为最新版，无需更新...")
            await bot.send_private_msg(
                user_id=int(list(bot.config.superusers)[0]),
                message=f"自动获取钟离版本成功：{latest_version}，当前版本为最新版，无需更新...",
            )
    else:
        logger.warning("自动获取钟离版本失败....")
        await bot.send_private_msg(
            user_id=int(list(bot.config.superusers)[0]), message=f"自动获取钟离版本失败...."
        )
    return 999, ''
...
def _file_handle(latest_version: str) -> str:
    ...
    logger.info("开始解压钟离文件压缩包....")
    tf = tarfile.open(zhenxun_latest_tar_gz)
    tf.extractall(temp_dir)
    logger.info("解压钟离文件压缩包完成....")
...
async def get_latest_version_data() -> dict:
    for _ in range(3):
        try:
            ...
            logger.error(f"检查更新钟离获取版本失败 {type(e)}：{e}")
    return {}
